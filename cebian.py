...
# 侧边栏导航
st.sidebar.title("侧边导航")

# 使用单选按钮作为主要导航
page_option = st.sidebar.radio(
    "选择页面",
    ["个人简历生成器", "大王相册", "南宁西乡塘美食", "音乐", "视频", "赵信"],
    index=0
)

# 根据选择显示页面
if page_option == "个人简历生成器":
    # ========== 简历编辑页面内容开始 ==========
    
    # 创建一个左侧边栏用于简历输入（但不是主侧边栏）
    with st.sidebar:
        # 清除之前的导航内容，专注于简历输入
        st.markdown("### 📋 简历信息填写")
        st.divider()
...
# 在侧边栏底部显示当前页面信息
st.sidebar.divider()
st.sidebar.info(f"当前页面: **{page_option}**")
