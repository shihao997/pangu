...
if page == "项目介绍":
    ...
    with col_left:
        ...
        with st.container():
            ...
            st.markdown("🔍 **专业分析**：多维度分析的详细统计分析")
            st.markdown("🧠 **智能预测**：基于学习特征创建的成绩预测")
...
# ===================== 2. 专业数据分析界面 =====================
    ...
    # 使用st.dataframe替代st.table，提供更好看的数据展示
    st.dataframe(
        major_stats[["专业", "每周平均学时", "期中平均分", "期末平均分"]],
        use_container_width=True
    )
    ...
    if not bd_major.empty:
        ...
        # 显示数值（转换出勤率为百分比）
        col_att, col_score = st.columns(2)
        with col_att:
            st.metric("平均上课出勤率", f"{bd_att*100:.1f}%")
        with col_score:
            st.metric("期末考试平均分", f"{bd_final:.2f} 分")
